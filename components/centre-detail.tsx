...
import { forecastPatientFootfall } from "@/lib/forecasting";
...
type TrendMetric = "beds" | "doctors" | "tests" | "footfall";
...
const footfallLineColor = "#5b5fc7";
...
function buildTrendData(centre: HealthCentre, status: ReturnType<typeof getCentreStatus>, metric: TrendMetric): TrendData {
  ...
  if (metric === "footfall") {
    return {
      ...
      subtitle: "Daily patient visits over the last 30 days for this centre.",
      unit: "patients",
      status: "good",
      data: centre.patientFootfall.slice(-30).map((point) => ({
        date: point.date.slice(5),
        value: point.count
      })),
      lineColor: footfallLineColor
    };
  }
...
export function CentreDetail({ centreId }: { centreId: string }) {
  ...
  const [activeMetric, setActiveMetric] = useState<TrendMetric>("beds");
  ...
  const lineColor = trend.lineColor ?? severityStyles[trend.status].line;
  const footfallForecast = forecastPatientFootfall(centre);
  ...
  return (
    <main className="craft-page mx-auto max-w-7xl px-4 lg:px-8">
      ...
      <motion.section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5" variants={staggerContainer} initial="hidden" animate="visible">
        ...
        <Kpi label="Avg patient footfall" numericValue={footfallForecast.recentAverage} decimals={0} />
      </motion.section>
      ...
      <motion.section className="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_390px]" variants={staggerContainer} initial="hidden" animate="visible">
        <div className="craft-card flex min-h-[440px] flex-col p-5">
          ...
          <div className="mt-4 flex min-h-72 flex-1 items-center overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.div
                key={activeMetric}
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trend.data} margin={{ top: 8, right: 12, left: -18, bottom: 0 }}>
                    ...
                    <Tooltip
                      formatter={(value) => {
                        const label = trendTabs.find((tab) => tab.value === activeMetric)?.label ?? "Value";
          ...
          {activeMetric === "footfall" ? (
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div className="craft-card-muted p-3">
                <p className="text-xs font-extrabold uppercase text-slate-500">Expected tomorrow</p>
                <p className="craft-number mt-2 text-3xl font-extrabold leading-none text-[#17212b]">~<AnimatedNumber value={footfallForecast.expectedTomorrow} /> <span className="text-sm font-bold tracking-normal text-slate-500">patients</span></p>
              </div>
              <div className="craft-card-muted p-3">
                <p className="text-xs font-extrabold uppercase text-slate-500">Two-day outlook</p>
                <p className="craft-number mt-2 text-3xl font-extrabold leading-none text-[#17212b]">~<AnimatedNumber value={footfallForecast.expectedDayAfterTomorrow} /> <span className="text-sm font-bold tracking-normal text-slate-500">patients</span></p>
                <p className="mt-1 text-xs text-slate-500">Recent average {footfallForecast.recentAverage} patients/day</p>
              </div>
            </div>
          ) : null}
        </div>
...
function Kpi({ label, value, numericValue, suffix = "", decimals = 0 }: { label: string; value?: React.ReactNode; numericValue?: number; suffix?: string; decimals?: number }) {
  return (
    <motion.div
      className="craft-card craft-lift p-4 lg:p-4"
      variants={riseIn}
      transition={entranceTransition}
      whileHover={{ y: -4, scale: 1.01 }}
    >
      <p className="min-h-8 text-[0.68rem] font-extrabold uppercase leading-4 text-[#5c6873]">{label}</p>
      <div className="craft-number mt-6 bg-gradient-to-br from-[#17212b] to-[#164e63] bg-clip-text text-3xl font-extrabold leading-none text-transparent lg:text-3xl xl:text-4xl">{typeof numericValue === "number" ? <AnimatedNumber value={numericValue} suffix={suffix} decimals={decimals} /> : value}</div>
    </motion.div>
  );
}
